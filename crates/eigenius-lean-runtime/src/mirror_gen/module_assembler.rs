...
fn lakefile_content() -> String {
    // D30 §2.1 — pinned package metadata. The require uses a
    // `path = "..."` form when the spec lands; for now we ship the
    // git-tagged form per the spec text. Production builds will
    // template the path via the substrate composer (Phase 20a.6.x).
    format!(
        "-- Auto-generated by eigon-ffi-gen — DO NOT EDIT.\n\
         ...
         require EigeniusLeanCommon from git\n  \
           \"https://github.com/eigenius/EigeniusLeanCommon.git\" @ \"{COMMON_PACKAGE_TAG}\"\n\
         \n\
