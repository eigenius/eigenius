...
- **[ESL — Eigenius Surface Language](docs/guides/esl/README.md)** — eleven chapters on the declarative surface (`namespace`, `class`, `property`, `resource`, `axiom`, `def`, `data`, `program`) and the ML-style expression sublanguage. Most important chapter: [chapter 6 — Resources, types, and the layer](docs/guides/esl/06-resources-types-and-the-layer.md), the bridge between the resource graph and the kernel's type theory.
...
- Run programs as resumable tasks with checkpointing
- Resolve ontology classes as kernel types on demand via the layer chain (Phase 10, D18)
- Type-check inductive declarations against the kernel's admission gate — strict positivity, universe-polymorphic recursor motives, and constructor arity — on the commit path (D19, D46, eigenius#92)
...
- **Tasks** — programs run as tracked tasks with checkpointing, positional trace keys, and a startup resume sweep for crash recovery.
...
| [D11: Codata and Streams](docs/design/d11-codata-streams.md) | ⚠️ **Superseded** (eigenius#218) — the codata half is retired; §5.3/§5.4 (task state, concurrency) survive |
...
| [D19: Inductive and Sized Types](docs/design/d19-inductive-types.md) | Inductive types and indexed families. The sized-termination and codata halves are retired (eigenius#218) |
