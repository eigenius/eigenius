...
The kernel is **Mini-TT** — a normalisation-by-evaluation type-checker for a dependent type theory with universes, Π-types, Σ-types, inductive types, coinductive types, sized types, and identity types. The implementation is in [`kernel/src/nbe/`](../../../kernel/src/nbe/). Two key files: [`term.rs`](../../../kernel/src/nbe/term.rs) defines `Exp` (syntactic terms), [`val.rs`](../../../kernel/src/nbe/val.rs) defines `Val` (semantic values).
...
- [D19 — Inductive and sized types](../../design/d19-inductive-types.md): the design specification, with the formal positivity check, sized recursion rules, and recursor derivation.
