...
**Eleven chapters covering**: HCL-style declarations (`namespace`, `class`, `property`, `resource`, `data`, `program`, plus `axiom` for postulated propositions, `macro` for compile-time smart constructors, and multi-class `data` headers for marker classes); the ML-style expression sublanguage (`let`, lambdas, pattern match, constructor application, projection, `formula(...)` and `type_expr(...)` typed-expression sublanguages); the bridge between the resource graph and the kernel's type theory (including the D49 chain-witness predicates the justification vocabulary consumes); the four capability modes (`Pure`/`Read`/`Check`/`IO`); the type-theory primer covering Π/Σ/inductive types plus the D46 unified Sort ladder with `Prop` and the D48 indexed-family elaboration; institution-dispatched decide predicates and comorphism program-invocations, plus a dedicated section on the D39 justification vocabulary and one on the protocol for hosting a logic; common error messages.
...
The cross-cutting story none of the per-host chapters tell on their own — what happens when *several institutions* cooperate, either through declared comorphisms over a shared payload language or through the per-layer chain-witness admission over shared proposition slots. The kinase-institutions notebook is the comorphism-shape worked example (five Julia institutions, three cross-institution comorphisms); the [drug-screening fixture](../../kernel/tests/fixtures/drug_screening.esl) is the witness-index-shape worked example (a D52 statistics recompute grounding a D39 conclusion through `App(Declared(plan), Observed(input))`).

**Ten chapters covering**: the three layers of composition (shared payload / declared comorphisms / coordinated dispatch roles); shared payload languages and identity-comorphism collapse, with a dedicated section on `eigentt:Term` as the witness-index-side shared payload; the triadic structure of comorphisms with the four-step dispatch pipeline; the three dispatch roles (AutoOnLoad / OnDemand / Decidable) in concert, including the D52 → D39 AutoOnLoad cascade; chain reinsertion of comorphism outputs through both surfaces; end-to-end walkthroughs of both composition shapes (kinase notebook in chapter 6, statistics + reasoning in chapter 7); composition patterns and anti-patterns; cross-composition failure modes; appendix.
...
- [D39 Justification Logic](../design/d39-justification-logic.md), [D49 Chain-witness machinery](../design/d49-chainwitness-machinery.md) — the justification vocabulary and its grounding mechanism, checked by the kernel at commit rather than by an institution (P7), surfaced in ESL §6.4a / §9.10 and walked through in [`platform/justification-logic/`](platform/justification-logic/)

- [D52 Measurement-statistics institution](../design/d52-measurement-statistics-institution.md) — the statistical-claim recomputation institution, walked through in [`platform/statistics-institution/`](platform/statistics-institution/); composes with D39 via witness admission (composition guide §7)
