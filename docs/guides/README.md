...
**Eleven chapters covering**: HCL-style declarations (`namespace`, `class`, `property`, `resource`, `data`, `program`, plus `axiom` for postulated propositions, `macro` for compile-time smart constructors, and multi-class `data` headers for marker classes); the ML-style expression sublanguage (`let`, lambdas, pattern match, constructor application, projection, `formula(...)` and `type_expr(...)` typed-expression sublanguages); the bridge between the resource graph and the kernel's type theory (including the D49 chain-witness predicates the reasoning institution consumes); the four capability modes (`Pure`/`Read`/`Check`/`IO`); the type-theory primer covering Π/Σ/inductive types plus the D46 unified Sort ladder with `Prop` and the D48 indexed-family elaboration; institution-dispatched decide predicates and comorphism program-invocations, plus a dedicated section on the D39 reasoning institution; common error messages.
