...
**Ten chapters covering**: the three layers of composition (shared payload / declared comorphisms / coordinated dispatch roles); shared payload languages and identity-comorphism collapse, with a dedicated section on `eigentt:TypeExpr` as the witness-index-side shared payload; the triadic structure of comorphisms with the four-step dispatch pipeline; the three dispatch roles (AutoOnLoad / OnDemand / Decidable) in concert, including the D52 → D39 AutoOnLoad cascade; chain reinsertion of comorphism outputs through both surfaces; end-to-end walkthroughs of both composition shapes (kinase notebook in chapter 6, statistics + reasoning in chapter 7); composition patterns and anti-patterns; cross-composition failure modes; appendix.
