...
- **`rhs: resource_array<RhsComponent>`** — a list of typed wrappers. `RhsComponent` is a one-property class wrapping a FormulaTerm. The wrapper originally existed because the chain had no parametric `core:List<T>` shape. **That reason has lapsed** — `core:List` is a chain-declared `InductiveType` with a type parameter as of eigenius#229 ([D79 §2.1.2](../../../design/d79-the-representation-of-inductive-types.md)), so a list-valued term slot is now expressible. The wrapper stays for its second reason: it leaves room for per-component metadata to grow later (provenance refs, doc strings, etc.) without changing the FormulaTerm payload.
