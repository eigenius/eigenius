...
The `Eigen` class has 33 public methods — 30 async plus three synchronous accessors (`getEndpoint`, `getDefaultBranch`, `useBranch`). Async responses are protobuf message objects (camelCased fields).

The table below covers the eleven the notebook MVP drives. The rest are grouped after it; the authoritative list is the class itself.
...
The remaining twenty-two, by area:

| Area | Methods |
|---|---|
| Client state (sync) | `getEndpoint`, `getDefaultBranch`, `useBranch` |
| Traces | `reflect` |
| Branches | `listBranches`, `getBranch`, `createBranch`, `deleteBranch`, `mergeBranches` |
| Merge resolution | `previewMerge`, `prepareMerge`, `previewCascade`, `submitResolution` |
| Consolidation | `estimateConsolidation`, `consolidateChain` |
| Tasks | `listTasks`, `getTaskStatus`, `cancelTask` |
| Tags | `createTag`, `listTags`, `deleteTag` |
| Garbage collection | `estimateGc`, `runGc` |

`prepareMerge` is worth singling out: it is the only surface in the whole platform that returns merge conflict ids, and the CLI has no equivalent subcommand. A caller resolving a merge from the CLI has to obtain the ids through this method, the raw `PrepareMerge` RPC, or the notebook.
...
└── examples/
    ├── smoke-test.ts     # 8 steps; covers the core RPCs, not every method
...
The smoke test at [`clients/eigenius-ts/examples/smoke-test.ts`](../../../clients/eigenius-ts/examples/smoke-test.ts) exercises the core RPCs against a live stack:
...
Output is an 8-step transcript (`TOTAL_STEPS = 8`): `health → inspect → query → listInstitutions → layerTopology (taxonomy + full) → load → validateProgram → branch CRUD + per-call routing`. This is the SDK's Phase 1 acceptance criterion (D22 §5.7).
