...
1. **Skip.** `LayerHandle::has_witness_candidates` is stamped at write time over the layer's resources. A layer holding no Trace, no `InstitutionEmittedDerivation` and no `ReasoningSentence` answers `false` with no probe at all — a lexicon layer stops here.
2. **Self-attesting.** `Layer::get_resource` on the key's IRI, which is layer-local. If that resource is a `ReasoningSentence` and the key's category is `Verified`, or an `InstitutionEmittedDerivation` and the category is `Derived`, build the key it would emit and compare it to the key asked for.
...
When the kernel encounters `JustifiedBy.derived(iri, P)` and needs to fill in `witness : IsDerivedAs(iri, P)`, `synthesize_chain_witness` runs the D49 §5 algorithm:
...
A miss returns a **free-form `String`**, not a structured error value. It names the predicate family (`IsDeclaredAs` / `IsObservedAs` / `IsDerivedAs` / `IsVerifiedAs`), the IRI, the property the resource would have to carry, and the `JustifiedBy.*` constructor that would become well-typed. There is no diagnostic enum anywhere on this path — see [the check](#the-four-step-validatejustification-check) below.
...
1. **Property decoding.** Read `reasoning:proposition`, `reasoning:justification` and `reasoning:certificate` from the sentence resource. The proposition and certificate are D47-encoded type-expression values, decoded to a kernel `Exp` by `decode_type`; the justification is lifted into a `Val::InductiveVal` typed at `reasoning:JustificationTerm` through the institution's own `extract_typed` route (`extract_justification`), so the chain-to-`Val` translation rides the standard institution surface. A property that is present but does not decode yields `Verdict::Fails` carrying `malformed proposition: ...` or `malformed certificate: ...`. A property that is *absent* does not reach a verdict at all: `required_property` returns an `InstitutionError::ComputationFailed`, on the reasoning that the class's `requires` enforcement has already rejected such a sentence at commit.
...
claim_eig0291_lowic50:result:main_effect        [StatisticalAnalysisResult +
  ...
  │
  ↑ App(DeclaredEvidence(rule),
  │     App(DeclaredEvidence(bridge), DerivedEvidence(result)))
concl_eig0291_strong                            [ReasoningSentence]
  ↑ ValidateJustification AutoOnLoad
Verdict("Holds")                                [the verdict]
```

The reasoning sentence's certificate is two nested [`JustifiedBy.app`](../../esl/09-institutions.md#9102-the-justifiedby-certificate-predicate) calls composing three sub-certificates. The inner `app` applies the bridge to the statistical result: `declared(bridge, lt(mean_of(s), 100.0) -> HasLowIC50(EIG_0291))` against `derived(result, lt(mean_of(s), 100.0))`, yielding `HasLowIC50(EIG_0291)`. The outer `app` applies the literature rule to that, yielding `StrongInhibitor(EIG_0291)`. Both grounding constructors are written with the trailing witness slot elided; the kernel fills each in.
...
- **`certificate does not type-check against JustifiedBy(justification, proposition): no admitted Is…As witness for IRI …`** — no layer in the resolution admits the cited `(category, iri, proposition)`. Four common causes:
...
- **The same prefix, with the kernel's own type error after it** — the certificate's shape does not match the justification's. Every mismatch of constructor, index or type arrives through this one path, so read the kernel's error: `JustifiedBy.observed` consumes `IsObservedAs`, which only an `ObservationTrace` admits, and using it to ground a `DeclaredEvidence(iri)` term is a category mismatch. Match the certificate constructor name to the justification's grounding-ctor name — `declared` for `DeclaredEvidence`, `derived` for `DerivedEvidence`, and so on.
...
- **`ReasoningSentence missing required … property`, arriving as an institution error rather than a verdict** — a required slot is absent. The class's `requires` enforcement should have rejected this at commit; reaching the handler means the institution was dispatched against a resource that did not come through the commit path.
