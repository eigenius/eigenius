...
> [ESL §9.11](../../esl/09-institutions.md#9-11-hosting-a-logic--the-operational-protocol) for the
> rule that decides it: *if the kernel can already check it, it is vocabulary, not an institution.*

Surface reference: [**ESL §9.10**](../../esl/09-institutions.md#9-10-the-justification-vocabulary--d39-justification-logic).
...
Reasoning leaves a typed audit trail. These shapes are pre-existing chain artifacts a conclusion cites, or that witness admission reads. (The check emits nothing at all now — a `Verdict` resource used to be its output, and P2 removed it along with the institution that produced it.)
...
One more shape carries the reasoning step itself:
...
The walk reuses the existing `Arc<Layer>` parent-chain walk (the one resource resolution uses) — no new traversal abstraction. First hit wins, except where a layer between the query point and the admitting layer rebinds a declaration the proposition depends on — then the credit does not carry (eigenius#227). Layer immutability keeps the RECORD stable, which is what the walk used to rest on, but it does not keep the MEANING of what was recorded stable: a descendant can redefine a name the proposition mentions, and widening a class it quantifies over makes the claim strictly stronger than the one that earned the credit. The `Val::ChainWitness` value carries no payload beyond the key; proof irrelevance ([ESL §7.1](../../esl/07-type-theory-primer.md#7-1-universes-the-unified-sortn-ladder-with-prop-at-the-bottom)) makes any two witnesses of the same `(category, iri, P)` definitionally equal.
...
[Rule 21](../../../../kernel/src/validation/rules/eigentt_value.rs) owns every
...
3. **`check(j.term, typ)`.** Run NbE `check` on the certificate against
   ...
   arrives as one string. The kernel's type error is what distinguishes them.
...
**There is no structured diagnostic taxonomy.** No `NoAdmittedChainWitness`,
`CertificateTypeMismatch`, `IndexMismatch`, `PropositionNotInProp` or `MalformedSentence` type
exists anywhere in the workspace. Those names appear in
[D49 §5](../../../design/d49-chainwitness-machinery.md) and D51 as *specified* shapes; they were
never built. Tooling that reads a failure must match on the message text, not on a variant. (This
guide asserted the taxonomy, and named a `diagnostic.rs` that never existed, until the section was
corrected on 2026-08-20; it described a four-step handler returning `Verdict::Fails` with a single
`urn:eigenius:institution:diagnostic` string until P7 deleted that handler on 2026-08-31.)
...
4. **Commit.** Load the fixture (`eigenius load <doc>`). Rule 21 checks the judgement as part of validating the layer; success → admit, failure → the commit is rejected and the validation error names what failed.
...
No bridge code, no manual handoff. The composition works because D52's emitted artifact carries `canonical_proposition` in the same slot as everything else, so a plan declaration can be written against it. Nothing on the justification side knows D52 exists; the check just sees `IsDeclaredAs` and `IsObservedAs` witnesses with the right hashes.
