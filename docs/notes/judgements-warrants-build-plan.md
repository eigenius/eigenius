...
> **Dissolved `2026-09-05`, not owed** (`kernel-run-records-build-plan.md` §2.3). This section set
> P4's shape to *"author 21 `DeclarationTrace`s (with agents and rationales)"* so that
> `Declared(plan)` resolves. That did not land, and it should not: it presumes the composite
> `App(Declared(plan), Observed(inputs))` is the operative shape for those plans.
>
> The paper's criterion is *"whether the plan formalizes a deterministic function, not the medium
> of execution"*, and for a real scientific pipeline that formalization is rarely available —
> eigenius#43 records why it is shaky even where asserted. Those results are **Sampled**: a bare
> `Observed` leaf. Consistent with the measurement, nothing on the chain cites a plan as
> `DeclaredEvidence`, so no consumer has ever wanted the witness this section proposed to mint.
>
> The plans remain declared documents. They simply do not carry the function declaration the
> composite would need, and minting 21 traces to satisfy a composite nobody uses would have been
> work in service of the wrong model.
...
### P6 is complete. The value-shape work is NOT a phase (`2026-08-31`)

P6.0 and P6.1 landed. There is no P6.2 in this plan and never was — the name was invented for
follow-on work sketched in an earlier session, and then treated as a plan item for two days.

That work is the SHAPE OF INDUCTIVE VALUES, specified in
[D85](../design/d85-the-shape-of-inductive-values.md). It is not sequenced here because it blocks
nothing: it grew from wanting a reference form to deduplicate a repeated term in one WRN file, which
is opportunistic cleanup. D85 §5 carries its own retrofit steps.

Two notes preceded D85 and are replaced by it —
[D83](../design/d83-inductive-value-wire-format.md), which found a real defect in the wire format
(two encodings that no document distinguished), and
[D84](../design/d84-an-inductive-value-is-a-resource.md), which found the model underneath it. Their
code was reverted in `5187cfc`; D85 §7 records why, and it is worth reading before starting: seven
unverified claims about the implementation drove work that had to be undone.
---
