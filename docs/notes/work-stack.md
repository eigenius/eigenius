...
> so the scoring semantics is checkable without a chain. Form one of the contract: it computes and
> asserts a proposition, mints no logic constant, earns no witness.
>
> **It asserts a proposition ABOUT the conclusion, never the conclusion.** A test decodes the
> emitted term and shows φ appearing as an argument of `Commits(policy, φ)` rather than being
> replaced. The bridge from one to the other stays a declared, attributed leaf, and nothing in the
> contract enforces that — a slot is checked for holding a well-formed proposition, never for which
> predicate it names.
>
> **Three things building it found.** The program AST has NO arithmetic — Apply, Lambda, Var,
> Literal, Case, Map, Reduce and Component, with every number multiplied inside a Component — so a
> chain-resident margin lambda is not expressible and δ is a declared form plus a coefficient. ESL
> cannot name a D85-materialised constructor class by qualified name, because the hyphen is not an
> identifier character; `institution:'Verdict-Holds'` already admits it. And the subject conclusion
> is scored like any other explanatory token, so the estimate and interaction slots admit a
> `Conclusion`.
>
> **A code review found five defects, two of which flipped a verdict.** A malformed interaction pair
> was dropped silently, deleting a rivalry and turning a suspension into a commitment. Conflicting
> estimates resolved by array position. No numeric input was range-checked, so a policy with ε ≥ τ
> validated and recorded no margins — which reads exactly like the genuine finding that a conclusion
> faced no live opposition. The margin clamp recorded a number the policy never named. And an
> unweighed COMPATIBLE alternative refused the whole assessment, calling it an active rival.
>
> **Two things removed rather than kept.** The general κ* lifting was wrong in both directions while
> looking general: the paper's structural equivalence is generated by commutativity and NOT
> associativity, so neither direction is a function of atom sets. Narrowed to the atomic case the
> vocabulary expresses. λ went with it — required on every policy, read into the institution, and
> consumed only by the synthesis operator, which nothing calls.
>
> **Lessons worth keeping.** A prose range is not a constraint: every number whose description said
> "in [0,1]" and declared no `min_value` accepted anything. Unreachable generality is not free. And
> a fixture edited by string replacement must assert its own edit matched, or a pattern that stops
> matching turns the test into a second copy of the unmodified case.
>
> **The pilot is one of three steps.** Step 2, the institution prototype, is done. Step 1 (rival
> reconstruction, weights, interaction estimates) and step 3 (the margin sweep and the
> re-adjudication table) are not started, and neither needs institution code: step 1 is authored
> chain data plus an embedding-based estimator that belongs in a `program:Component` rather than in
> the adjudicator, and step 3 is data generation plus a query over committed decisions. The δ ≡ 0
> reproduction gate runs over a FIXTURE, not the WRN chain, because the chain carries no
> plausibility scores — the specification assumes it does, and eliciting 33 weights is the one input
> the chain cannot supply. **33 conclusions, not the pilot's 52**; the 52 counts the narrative layer.
>
> **Open before anyone generates 600 resources.** A sweep is one policy per grid point and one
> assessment per (conclusion, policy), so its structure would exist only as a naming convention. A
> first-class sweep resource naming its policies and subjects would let the re-adjudication table be
