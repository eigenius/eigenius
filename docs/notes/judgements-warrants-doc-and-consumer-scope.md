...
| D58 objective framing (removed `2026-09-03` with the `objective:` layer) | 12 | §H2 records reusing `reflection:EpistemicStatus` for `objective:acceptance_grade`. See §6.1 below — this is a gap in the plan, not only a doc edit |
...
| [platform/justification-logic/README.md](../guides/platform/justification-logic/README.md) | 282 | 44 | ~~retire, do not rewrite~~ — **REWRITTEN `2026-08-31`, and the disposition's premise was wrong.** It said "every hop but the first changes" of the walkthrough (verdict → certificate → witnesses → traces → artifacts). Measured after P7: only the FIRST changed. The verdict is gone; certificate, witnesses, traces and artifacts are untouched, because what P7 moved was the witness types' DECLARATION SITE, not any admission behaviour. Retiring would have discarded a working end-to-end walkthrough, the authoring instructions and the troubleshooting section to fix one hop. The institution framing was rewritten in place, the directory renamed off `*-institution/` — which is the disposition's substantive point — and it stays under `platform/`, where `julia-institutions/`, `lean-institution/` and `statistics-institution/` establish the sub-directory pattern for a per-host deep dive |
...
**The guides are the natural last gate.** `composition/07` and `platform/justification-logic/README.md`
