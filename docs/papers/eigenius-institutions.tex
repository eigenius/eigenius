...
The kernel's ground type resolution interface---where Mini-TT~\cite{minitt} type checking resolves class references against the layer stack---is an institution comorphism from the Eigon structural institution to the Mini-TT institution.
Signatures translate forward (classes become dependent record types via NbE~\cite{nbe}); models translate backward (well-typed programs reduce to valid resource instances).

\subsection{Fiber Advertisement}

In the minimal implementation, institutions advertise their internal morphism types as ordinary ontology classes.
...
A richer extension---where the kernel exploits structural properties of fiber morphisms (e.g., ``mesh refinement is a preorder'')---would require the kernel to reason within notions defined by external institutions, violating the architectural boundary.
Instead, institutions may optionally expose a \emph{fiber reasoner}: a capability that answers structural queries about its own morphisms, dispatched by the kernel the same way it dispatches to the EigenQL evaluator or Mini-TT type checker.
The institution reasons about its own internal structure; the kernel remains the switchboard.
...
Four phases of Eigenius are implemented in Rust (kernel) and TypeScript (orchestration).
The Eigon data model~\cite{atomicdata} provides the shared signature category.
EigenQL (stratified Datalog with aggregation~\cite{alice}) serves as the query institution.
Mini-TT~\cite{minitt} dependent type checking via NbE~\cite{nbe} provides the program validation institution.
Persistent storage uses RocksDB with CBOR~\cite{rfc8949} serialization.
...
Reasoning traces~\cite{lean4} capture \emph{vertical} provenance (how outputs relate to inputs through computation steps) but not \emph{horizontal} fiber structure (how results within an institution relate to each other).
Epistemic status is computed correctly from trace provenance; the fiber-theoretic account explains \emph{why} the four categories are the right ones but does not yet change the implementation.
...
The richer extension---fiber reasoners as dispatched capabilities---follows the existing capability protocol pattern.
...
This appendix presents a sketch of the \code{FiberReasoner} trait---the interface by which an institution exposes reasoning about its internal fiber structure to the kernel.
