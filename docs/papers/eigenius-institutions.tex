...
\textbf{Mini-TT type checking} via NbE~\cite{minitt,nbe} validates program composition---function types, dependent pairs, pattern matching.
...
Mini-TT checks that a program \emph{composes}; Lean~4 checks that a result is \emph{correct}.
The distinction is operational: the kernel cannot function without Mini-TT; it functions without Lean~4.
...
The kernel's ground type resolution interface---where Mini-TT type checking resolves class references against the layer stack---is a kernel-internal comorphism from the Eigon structural base to the Mini-TT type system.
...
Mini-TT~\cite{minitt} dependent type checking via NbE~\cite{nbe} provides program validation as a kernel service.
...
Mini-TT has been extended with features inspired by Lean~4~\cite{lean4}:
...
These extensions close the gap between the Eigon ontology constraints and the Mini-TT type system without introducing refinement types or subtyping.
