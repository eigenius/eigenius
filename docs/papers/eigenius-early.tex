...
A dependent type theory (EigenTT) provides static guarantees for processing pipelines: termination, type safety, and exhaustive error handling.
...
\begin{enumerate}
  ...
  \item A program model grounded in EigenTT~\cite{minitt} dependent type theory providing static guarantees for processing pipelines;
...
Programs in Eigenius are typed expressions in a functional language grounded in EigenTT~\cite{minitt}, a minimal dependent type theory that is a direct fragment of CIC---the Calculus of Inductive Constructions underlying Lean~4~\cite{lean4}.
...
Twelve expression forms (Program, Let, Apply, Var, Lambda, Case, Pair, Construct, Project, Map, Reduce, Literal) map directly to EigenTT terms with no translation layer.
...
EigenTT dependent type theory ported from the Haskell reference implementation.
...
Eigenius uses EigenTT~\cite{minitt}---a minimal fragment of CIC---not as a general programming language but as the foundation for validating AI processing pipelines against ontology schemas.
