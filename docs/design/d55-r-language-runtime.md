...
**Open — the one remaining P5 step: wrapped-R output → reasoning chain.** Lifting `concl_vivo` from linked-external to recomputed needs the R `DerivedResource` (carrying the LRT p) to compose with the reasoning institution — i.e. carry a `reflection:canonical_proposition` + `IsDerivedAs` so a Declared bridge can `derived(RESULT, P)` it into `InVivoDependence(WRN, MSI)`, exactly as the statistics-institution results do. The marshalling builder currently sets scalar/array/string properties but not a typed `canonical_proposition` (a structured type-expr). Two candidate designs: (a) extend the builder with a `r_eigon_set_proposition` that takes a pre-encoded proposition (or builds the common `stats:lt(p_value_of(s), α)` shape), so the R output is self-describing; (b) a thin Rust adapter that reads the R output's `lrt_p_value` and emits a `StatisticalAnalysisResult`-shaped resource carrying the canonical proposition (reusing the statistics institution's machinery). This + a verified lme4 result (in the pinned image / an lme4-equipped host) closes `concl_vivo`. Deliberately not hacked here — it is a real design choice, and unverifiable without lme4.
