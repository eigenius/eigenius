...
The foundation is Eigon, a canonical typed data format built on three primitives: Class (what a thing is), Property (what it has), and Datatype (what values look like). These primitives are self-describing — Class is itself an instance of Class — creating a fixed point from which all other ontology definitions are derived. Domain knowledge, processing pipelines, capability registrations, reasoning traces, and proof terms are all expressed in Eigon. The knowledge graph is not a separate database that the system writes to; it *is* the system's representation of itself and the world.
