...
| `Value::Vector` | in the persisted value type; panics if serialised | **removed `2026-09-02`** — out of `Value` (§6) |
...
| 6 | **DONE `2026-09-02`.** `Vector` leaves `Value`. It needed no domain extension after all: nothing constructed the variant. D43 §4.1 describes an `EMBED` primitive whose output flows into `VECTOR_NEAR` / `VECTOR_SIM`, but `EMBED` never became a query function — the similarity evaluator calls the `Embedder` and works on the `Vec<f32>` it returns, and the chain stores vectors as `vec_seg:<I>:<L>` blobs (§2.4). What the variant actually held was a transient in the persisted value type, defensible only by the two panics that guarded canonical CBOR and Eigon-JSON. Both are gone with it, along with `as_vector`, the `values_equal` arm, and the four walkers' no-op arms. A vector-valued query domain, when `EMBED` lands, extends the query engine's value type — not the data model's. | yes |
