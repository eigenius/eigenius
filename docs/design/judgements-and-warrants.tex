...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{name} & \textbf{shape} & \textbf{ground set} & \textbf{condition} \\
\midrule
\emph{Computed} & $\mathrm{App}(\mathrm{Declared}(f : I \to O), \mathrm{Observed}(\mathit{input}))$ & $\{\mathit{Declared}, \mathit{Observed}\}$ & the plan is declared a function \\
\emph{Sampled} & a bare $\mathrm{Observed}$ leaf & $\{\mathit{Observed}\}$ & no $I \to O$ is assertable \\
\bottomrule
\end{tabular}
\end{table}
...
Neither shape yields a \emph{Verified} ground: the ground set of each contains no checked judgement, and in \emph{Computed} the typing $f : I \to O$ is itself declared rather than proven. Strengthening both properties changes the ground. Where the typing is dependent, $f : (i : I) \to O(i)$, and is proven rather than asserted, and the input commits as a term, the application $f\,i : O(i)$ is checked by the kernel and establishes a \emph{Verified} ground. Because satisfaction is inhabitation (\S\ref{sec:inhabitation}), evaluation and proof coincide in this case. The proposition so established is indexed by that specific input and does not generalize over $I$; a general statement over $I$ remains a separate obligation.
