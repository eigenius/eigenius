...
- **Phase 4.5 — `SplitPlot`.** Not yet landed. Two-factor design with hard-to-randomize whole-plot factor + easily-randomized subplot factor; needs nested error strata (whole-plot error larger than subplot error). The verifier reports two F-tests — whole-plot F against whole-plot error and subplot F (plus interaction) against subplot error. The high-priority false-positive shield D52 §5.2 calls out: split-plot misidentified as factorial silently produces wrong error terms.

- **Phase 4.9 — `RepeatedMeasures`.** Not yet landed. Longitudinal design with autocorrelation. Most algorithmic complexity (generalized least squares with the asserted covariance structure — AR(1) / CompoundSymmetry / Unstructured per the §12 decision); approximate F-tests; REML-flavored variance estimation. The autocorrelation structure is author-asserted per §12 #5 (claim's `autocorrelation_structure` field) so the verifier doesn't have to fit it iteratively.
