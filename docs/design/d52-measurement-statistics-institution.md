...
- **Phase 4.9 — `RepeatedMeasures` (CompoundSymmetry only). ✅ LANDED.** New `stats:RepeatedMeasures(n_subjects, n_timepoints, observations, replication)` smart constructor at product position `(CompleteRandom, Unblocked, SingleFactor, _, Longitudinal(n_timepoints))`. The `Longitudinal(T)` axis on the fifth slot distinguishes this dispatch from cross-sectional `IID` — both share `(CompleteRandom, Unblocked, SingleFactor)` on the first three axes, but only RM carries `Longitudinal`. Observations encoded as `[subject_id, time_idx, value]` rows (3 floats per observation, total `3 · n_subjects · n_timepoints`). The verifier reads `autocorrelation_structure` from the claim per §12 #5 and dispatches: `CompoundSymmetry` (or absent, defaulting) routes to `repeated_measures_cs_anova` — univariate RM-ANOVA with subject as random block, time as fixed factor; algebraically equivalent to RCBD with subject = block, time = treatment. `AR1` and `Unstructured` return `Fails` verdicts with a "not yet wired" diagnostic naming the asserted structure — those need GLS / MANOVA-style numerics that don't slot cleanly into the RCBD scaffold. The treatment (time) F-test is reported in the existing `(computed_statistic, computed_p_value)` slots; the verdict's diagnostic note names the autocorrelation structure used for audit. SS_error computed via the per-cell residual formula under the additive subject + time model (same catastrophic-cancellation-avoidance approach RCBD and SplitPlot take). Integration test: 5-subject × 4-timepoint drug clearance design with clean monotone decline (~-20 units/timepoint) → F_time > 100, p ≪ 1e-6, clear Holds with `CompoundSymmetry` named in the diagnostic; AR1 path returns the documented "not yet wired" Fails. Phase 4.9.5+ would land the GLS-based AR1 + Unstructured dispatches when chain-author pull justifies the numerics investment.

Order can flex on real chain-author pull; Phase 4.0 is the prerequisite for both 4.5 and 4.9 because they share the two-way-ANOVA scaffold.

**Phase 5 — Opinionated-stance hardening.** Outlier-exclusion dual-verdict commit shape, Passing-Bablok-mandatory for `MethodComparisonClaim`, impossibility-witness validation for `One_Sided_Witnessed`. These were skeletal in Phase 1; this phase makes them load-bearing rejections. Lands once at least Phase 2 is real and we know the surface ergonomics.
