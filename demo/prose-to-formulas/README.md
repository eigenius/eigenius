...
|---|---|---|---|
| **pinned literature rule** | a published `∀m. A → B` on the chain, specialized with `instantiate` and applied to a claim a sentence established | Declared | one rule, reused |
...
3. **Resolve** the `justification:Grounds` inductive declaration from the layer.
4. **Type-check the proposition at `Prop`** (`Sort(0)`) and evaluate it.
5. **Construct the expected type** — the crux:

   ```text
   justification:Grounds(justification_val, proposition_val)
   ```

   `justification:Grounds : justification:Term -> Prop -> Type 0` is an **indexed** inductive, so the
...
app( A, B,
     ...
     instantiate( Set, (fun m => A(m) → B(m)), Declared(rule),
...
`instantiate` eliminates the quantifier: from `justification:Grounds(j, ∀y:T. P(y))` it yields
`justification:Grounds(SpecStr(j, tag), P(x))`. `app` then composes: from `j1 : justification:Grounds(_, A → B)` and
`j2 : justification:Grounds(_, A)` it yields `justification:Grounds(App(j1,j2), B)`. **`A` must be the same term on both
...
`justification:Grounds(DerivedEvidence(claim_1), P)`, `app` cannot be applied, and the certificate fails against
`justification:Grounds(justification, C)`.
...
`instantiate` and `app` compose them and the result commits at **Declared** — a conclusion is no
...
No rule produces `justification:Grounds(_, A → B)` — `app` yields `B`, `sum_l`/`sum_r` yield `P`,
`instantiate` yields `P(x)`. An implication enters only through a grounding, so a rule
connecting propositions must be asserted; it cannot be proved.

## Appendix: `instantiate`
...
instantiate :
    ...
    justification:Grounds(j, forall (y : T) => P(y)) -> justification:Grounds(SpecStr(j, tag), P(x)),
```
...
subsumed (eigenius#203). `instantiate` binds the domain type and the instance on the *proof* side
while the justification term carries only a string tag, so `justification:Term` needed no change;
only `justification:Grounds` gained a constructor. The domain binder is `Type 1`, not `Set`: a rule whose own
