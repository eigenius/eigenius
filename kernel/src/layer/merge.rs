...
pub fn classify_iri_disagreement(
    ...
    if kind_a == ResourceKind::Property {
        if let Some(conflict) =
            check_property_data_type(iri, &resource_a, &resource_b, span, backend)?
        {
    ...
    if !resource_bodies_equal(&resource_a, &resource_b) {
        let ancestor_body = load_ancestor_body(iri, span, backend)?;
...
fn check_property_data_type(
    ...
    match (type_a, type_b) {
        (Some(a), Some(b)) if a != b => {
            let ancestor_body = load_ancestor_body(property, span, backend)?;
...
/// if the IRI doesn't exist at the ancestor (i.e., both branches
/// introduced it fresh) — distinct from a storage error, which
/// propagates.
fn load_ancestor_body(
    ...
    // Walk parents from the ancestor head down until we find a layer
    // that defines `iri`. The topology walk is bounded by the
    // ancestor's depth, which is finite by construction. For v1 we
    // do a simple ResourceBackend probe at the ancestor itself; the
    // backend's chain-walking lookups handle ancestor reachability.
    match backend.try_load_resource(&span.ancestor, iri)? {
        Some(resource) => Ok(Some(ResourceBody {
            source_layer: span.ancestor.clone(),
            resource,
        })),
        None => Ok(None),
    }
...
pub fn detect_inheritance_cycles(
    ...
    let merged_graph = build_merged_subclass_graph(span, backend)?;
...
fn build_merged_subclass_graph(
    ...
    // either branch contributed but didn't itself redeclare — those
    // arrows survive into the merge unchanged. We probe the ancestor
    // layer for every contributed class IRI; missing-resource is
    // simply "branch added a fresh class with no ancestor arrows."
    ...
    for iri in &contributed {
        let ancestor_res = match backend.try_load_resource(&span.ancestor, iri)? {
            Some(r) => r,
            None => continue,
        };
...
/// Yield every `Iri` referenced by a `Value` — single ResourceRef,
/// or any number through an Array of ResourceRefs. Other value shapes
/// (literal scalars, embedded objects) yield nothing; `subclass_of`
/// only carries IRIs in practice.
fn iter_iri_values(value: &crate::ontology::resource::Value) -> Vec<Iri> {
    use crate::ontology::resource::Value;
    match value {
        Value::ResourceRef(iri) => vec![iri.clone()],
...
pub fn classify_conflicts(
    ...
    for iri in span.shared_iris() {
        if let Some(kind) = classify_iri_disagreement(span, &iri, backend)? {
            let id = match &kind {
    ...
    for kind in detect_inheritance_cycles(span, backend)? {
        let id = match &kind {
            ConflictKind::InheritanceCycle { cycle } => {
                ConflictId::from_iris("inheritance_cycle", cycle)
            }
...
mod tests {
    ...
    fn inheritance_cycle_detected_when_branches_combine() {
        ...
        let cycles = detect_inheritance_cycles(&span, &backend).unwrap();
        assert_eq!(
            cycles.len(),
            1,
