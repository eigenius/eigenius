...
/// Comorphism — emits
/// `Exp::InstitutionInvoke { comorphism_iri, source }`.
///
/// The produced resource gets a deterministic content-hash IRI at evaluation time. The
/// decoder read a `urn:eigenius:program:target_iri` override until eigenius#149 removed it:
/// the property was never declared in the program ontology, this was its only mention in
/// the tree, and the lowering that builds a `ComorphismInvokeApply` sets `function` and
