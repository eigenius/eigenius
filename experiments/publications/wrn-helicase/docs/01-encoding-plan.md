...
Each node is a candidate `justification:Conclusion` whose `justification:Term` cites the Observed/Derived/Declared resources beneath it. `C-MAIN` is the paper's thesis; its justification is the composition of the whole graph.
...
Nodes from §1; each edge `A ⇐ B` reads "A is justified by B". This is the `justification:Term` skeleton.
