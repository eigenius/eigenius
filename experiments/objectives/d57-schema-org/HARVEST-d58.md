...
`justification:Sentence` (`concl_discipline`) whose proposition is *the same*
`Prop` as the milestone (`MappingDisciplineDefined("schema_org")`). The "milestone
as a `justification:Sentence` stub, completed when it Holds" candidate (D58 §5) works:
the stub carries the proposition + acceptance metadata; discharge fills in
justification + certificate. So `objective:Milestone` can *be* a justification:Sentence
...
edges (H1); `objective:Milestone` = a `justification:Sentence` stub + acceptance
